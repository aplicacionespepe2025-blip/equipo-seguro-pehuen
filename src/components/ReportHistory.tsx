...
export const ReportHistory: React.FC = () => {
  ...
  const reportesFiltrados = useMemo(() => {
    return reportes.filter((r) => {
      ...
      // 2. Filtro por Nombre de Evaluador o Receptor (Búsqueda en Mayúsculas)
      if (evaluadorBusqueda.trim()) {
        ...
        const receptorUpper = (r.NombreReceptor || '').toUpperCase();
        if (!evalUpper.includes(queryUpper) && !receptorUpper.includes(queryUpper)) return false;
      }
              ...
              >
                <div>
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    {/* EVALUADOR & RECEPTOR */}
                    <div className="bg-[#EFEAD0] p-3 rounded-xl border border-[#D1CB9E] space-y-1.5">
                      <div>
                      ...
                      {r.NombreReceptor && (
                        <div className="border-t border-[#D1CB9E] pt-1.5">
                          <div className="text-[10px] font-bold uppercase text-[#676057] tracking-wider">
                            RECEPTOR:
                          </div>
                          <div className="font-heading font-semibold text-xs text-[#3E3933] uppercase break-words">
                            {r.NombreReceptor.toUpperCase()}
                          </div>
                        </div>
                      )}
                    </div>
                ...
                {reportesFiltrados.map((r) => {
                  return (
                    <tr key={r.id || r.Id_Evento} className="hover:bg-[#EFEAD0] transition-colors font-medium">
                      ...
                      <td className="p-3.5 font-semibold uppercase text-[#676057]">{r.NombreReceptor ? r.NombreReceptor.toUpperCase() : '-'}</td>
