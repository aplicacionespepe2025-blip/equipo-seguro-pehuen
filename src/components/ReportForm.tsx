...
export const ReportForm: React.FC<ReportFormProps> = ({ onSuccessSave }) => {
  ...
  const [fecha, setFecha] = useState<string>(todayDateStr);
  ...
  // NUEVO CAMPO: Nombre Receptor (SIEMPRE EN MAYÚSCULAS)
  ...
  // Manejo de cambio en Nombre Receptor (Transforma instantáneamente a MAYÚSCULAS)
  const handleNombreReceptorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const upperValue = e.target.value.toUpperCase();
    setNombreReceptor(upperValue);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // RESTRICCIÓN DE FECHA: No permitir fechas superiores a la fecha actual del sistema
    const currentSystemDate = getTodayDateStr();
    if (fecha > currentSystemDate) {
      setErrorMessage(`La fecha de la evaluación (${fecha}) no puede ser superior a la fecha actual del sistema (${currentSystemDate}).`);
      return;
    }
    ...
    try {
      ...
      const nuevoReporte: Omit<ReporteSeguridad, 'id'> = {
        ...
        NombreReceptor: nombreReceptor.trim().toUpperCase() || 'NO ESPECIFICADO',
        Turno: turno,
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-[#FAF8EA] border border-[#D1CB9E] rounded-2xl p-6 space-y-5 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {/* Campo NOMBRE DE RECEPTOR con transformación a MAYÚSCULAS en tiempo real */}
            <div className="md:col-span-2 space-y-2 bg-[#FAF8EA] p-4 rounded-xl border border-[#D1CB9E]">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-[#3E3933] uppercase tracking-wide flex items-center space-x-1">
                  <User className="w-3.5 h-3.5 text-[#8A8602]" />
                  <span>Nombre Receptor</span>
                  <span className="text-[#80776D] text-[11px] font-normal font-sans">(Persona que recibe la retroalimentación)</span>
                </label>
                <span className="text-[10px] bg-[#EFEAD0] text-[#676057] px-2 py-0.5 rounded font-mono font-bold uppercase border border-[#D1CB9E]">
                  AUTO-MAYÚSCULAS
                </span>
              </div>
              <input
                ...
                onBlur={(e) => setNombreReceptor(e.target.value.toUpperCase())}
                placeholder="EJ. CARLOS EDUARDO ROJAS (RECEPTOR EN TERRENO)"
                className="w-full bg-[#FAF8EA] border border-[#676057]/40 rounded-xl px-4 py-2.5 text-[#3E3933] placeholder-[#80776D] focus:outline-none focus:ring-2 focus:ring-[#BCB703] font-heading font-bold tracking-wide uppercase text-sm"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-[10px] text-[#8A8602] font-semibold">
                * Restricción: No se permiten fechas futuras superiores a hoy ({todayDateStr}).
              </p>
            </div>
